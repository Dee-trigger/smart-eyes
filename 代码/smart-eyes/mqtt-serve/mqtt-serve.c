...
// 发布 设备属性数据的主题
#define MQTT_TOPIC_PUB_HEART-RATE "$oc/devices/%s/user/heart_rate"
#define MALLOC_MQTT_TOPIC_PUB_HEAT "$oc/devices/user/heat"
...
void mqtt_send_task(void)
{
    while (1) {
        // 获取传感器的数据
        SHT20_ReadData(&sensorData.heart_rate, &sensorData.heat);
